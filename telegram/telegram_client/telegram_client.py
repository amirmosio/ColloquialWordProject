...
from exceptions import handle_my_exceptions_print
...
class ResearchTelegramClient:
    ...
    def __init__(self, language_model_service):
        ...
        self.preprocess_utils = self.language_model_service.word2vec_service.provider.preprocess_utils

        @handle_my_exceptions_print()
        @self.client.on(events.NewMessage())
        async def handler(event):
            if not ((event.is_group or event.is_channel) and not event.is_private):
                return
            text = event.raw_text
            ...
            for sentence in text_sentences:
                if not (self.preprocess_utils.remove_english_char_from_text(sentence) == sentence and
                    ...
                    continue
                tokens_list = self.preprocess_utils.process_context(sentence)
                res = self.language_model_service.get_colloq_question_if_exists(tokens_list)
                if res:
                    token, scores = res
                ...
                self.language_model_service.train_model_with_new_sentence_and_save_eventually(tokens_list)
...
#
#     async with ResearchTelegramClient() as my_client:
